...
import { X, Heart, Share2, Download, MoreVertical } from 'lucide-react';
...
export function MediaDetail({ 
  ...
  const handleLike = useCallback(async () => {
    ...
    try {
      if (newLiked) {
        ...
        console.log('[MediaDetail] Liked successfully');
      } else {
        ...
        console.log('[MediaDetail] Unliked successfully');
      }
    } catch (error) {
      // Revert on error
  ...
  const handleSingleTap = (e: React.TouchEvent) => {
    ...
    // Don't toggle UI if tapping on buttons or interactive elements
  ...
  const handleDoubleTap = async (e: React.TouchEvent) => {
    ...
    // Show animation at tap location
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showUI && (
          <>
            ...
            <button
              ...
              className="absolute top-4 left-4 z-50 w-10 h-10 rounded-full bg-black/30 backdrop-blur-sm 
            ...
            <div className="absolute right-4 bottom-24 z-50 flex flex-col items-center gap-6">
              ...
              >
                <Heart 
                  className={`w-8 h-8 transition-all duration-200 drop-shadow-lg ${
                    isLiked 
        ...
        {/* Swipe Indicator (visible when UI shown) */}
        {showUI && currentIndex < allMedia.length - 1 && (
