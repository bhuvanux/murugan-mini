  ...
  return (
    <div className="space-y-6 text-inter-regular-14">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-inter-bold-20 text-gray-800">Wallpaper Management</h2>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-4 gap-6">
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <p className="text-gray-500 text-sm text-inter-regular-14">Total Wallpapers</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 relative overflow-hidden">
            ...
            <p className="text-3xl font-bold text-gray-800 mt-1 text-inter-bold-20">
              {aggregateAnalytics ? aggregateAnalytics.total_views.toLocaleString() : wallpapers.reduce((sum, w) => sum + (w.view_count || 0), 0).toLocaleString()}
            </p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 relative overflow-hidden">
            ...
            <p className="text-3xl font-bold text-gray-800 mt-1 text-inter-bold-20">
              {aggregateAnalytics ? aggregateAnalytics.total_downloads.toLocaleString() : wallpapers.reduce((sum, w) => sum + (w.download_count || 0), 0).toLocaleString()}
            </p>
            {aggregateAnalytics && startDate && endDate && (
              <p className="text-xs text-gray-500 mt-1.5 flex items-center gap-1">
                <CalendarIcon className="w-3 h-3" />
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 relative overflow-hidden">
            ...
            <p className="text-3xl font-bold text-gray-800 mt-1 text-inter-bold-20">
              {aggregateAnalytics ? aggregateAnalytics.total_likes.toLocaleString() : wallpapers.reduce((sum, w) => sum + (w.like_count || 0), 0).toLocaleString()}
            </p>
            {aggregateAnalytics && startDate && endDate && (
              <p className="text-xs text-gray-500 mt-1.5 flex items-center gap-1">
                <CalendarIcon className="w-3 h-3" />
              ...
              {filteredWallpapers.map((wallpaper) => (
                <div
                  ...
                  className={`bg-white rounded-xl shadow-sm border-2 overflow-hidden hover:shadow-md transition-all ${
                    selectedWallpapers.has(wallpaper.id) ? 'border-green-500' : 'border-gray-200'
                  }`}
                >
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-gray-200">
                      ...
                      {wallpaper.publish_status !== "scheduled" && (
                        <button
                          ...
                          className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg transition-colors text-inter-medium-16 ${
                            wallpaper.publish_status === "published"
              ...
              {filteredWallpapers.map((wallpaper) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    {/* Status Badge or Countdown Timer */}
                    <div className="flex items-center gap-2">
                      {wallpaper.publish_status === "scheduled" && wallpaper.scheduled_at ? (
                        <>
                          <CountdownTimerBadge
