...
export async function getSparkleAnalytics(c: Context) {
  try {
    ...
    const { data: sparkle, error: sparkleError } = await supabase
      .from("sparkles") // [FIX] Updated to correct table name 'sparkles'
    ...
    // Use cached counts from Sparkle table as source of truth (synced by trigger)
    // Fallback to calculated values if cached values are 0 (e.g. before backfill)
    ...
    // Prefer cached counts, but use tracked counts if higher (e.g. sync lag or migration issue)
    const totalViews = Math.max(sparkle.view_count || 0, lifetimeViews || 0);
    const totalReads = Math.max(sparkle.read_count || 0, lifetimeReads || 0);
    const totalDownloads = Math.max(sparkle.download_count || 0, lifetimeDownloads || 0);
    const totalLikes = Math.max(sparkle.like_count || 0, lifetimeLikes || 0);
    const totalShares = Math.max(sparkle.share_count || 0, lifetimeShares || 0);
