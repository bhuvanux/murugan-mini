...
export async function trackEvent(c: Context) {
  try {
    ...
    const { data, error } = await supabase.rpc("track_analytics_event", {
      ...
      p_metadata: metadata,
      p_user_id: user_id || null // Pass user_id explicitly
    });
 ...
 * POST /api/analytics/track-install
 * Body: { device_id, platform, os_version, model, manufacturer, app_version, metadata? }
 */
export async function trackInstall(c: Context) {
  try {
    const body = await c.req.json();
    const {
      device_id, platform, os_version, model,
      manufacturer, app_version, metadata = {}
    } = body;
    ...
    const supabase = supabaseClient();
    const ipAddress = getClientIP(c);
    const location = getLocationInfo(c);

    console.log(`[Analytics] Tracking install for device ${device_id} on ${platform} from ${location.country}`);
    ...
    const { data, error } = await supabase
      ...
      .upsert({
        ...
        country: location.country_name || location.country, // Fallback to code if name missing
        country_code: location.country,
        region: location.region,
        city: location.city,
        last_active_at: new Date().toISOString(), // Mark active on install
        metadata,
    ...
    if (error) {
      console.error("[Analytics] Track install error:", error);
      return c.json({ success: false, error: error.message }, 500);
    }

    // Also track as a general event for unified analytics if needed
    try {
      await supabase.rpc("track_analytics_event", {
        p_module_name: 'app',
        p_item_id: '00000000-0000-0000-0000-000000000000', // System level ID
        p_event_type: 'install',
        p_ip_address: ipAddress,
        p_user_agent: c.req.header("user-agent") || null,
        p_device_type: getDeviceType(c.req.header("user-agent")),
        p_metadata: { device_id, platform, ...metadata },
      });
    } catch (e) {
      console.warn("[Analytics] General install event tracking failed (non-fatal):", e);
    }

    return c.json({ success: true, install: data });
  } catch (error: any) {
    console.error("[Analytics] Track install error:", error);
    return c.json({ success: false, error: error.message }, 500);
  }
...
export async function getMediaAnalytics(c: Context) {
  try {
    ...
    // 2. Get Events from Analytics Tracking (filter by media module names)
    ...
    // 3a. Get Lifetime Totals from Analytics Tracking
    // We calculate these on-the-fly to ensure they match the list view aggregation
    const { count: lifetimePlays } = await supabase
      .from("analytics_tracking")
      .select("id", { count: "exact", head: true })
      .eq("item_id", mediaId)
      .in("module_name", ["media", "song", "video"])
      .in("event_type", ["play", "play_video_inline"]);
    ...
    const { count: lifetimePlaylistAdds } = await supabase
      .from("analytics_tracking")
      .select("id", { count: "exact", head: true })
      .eq("item_id", mediaId)
      .in("module_name", ["media", "song", "video"])
      .eq("event_type", "add_to_playlist");

    const { count: lifetimeYoutubeOpens } = await supabase
      .from("analytics_tracking")
      .select("id", { count: "exact", head: true })
      .eq("item_id", mediaId)
      .in("module_name", ["media", "song", "video"])
      .in("event_type", ["youtube_open", "open_in_youtube"]);
    ...
    const responseData: any = {
      ...
      // All-time totals from analytics_tracking (Real-time)
      total_views: (media.view_count || media.views || 0) + (lifetimePlays || 0), // Use DB views + plays as rough estimate if needed, or just plays
      total_plays: lifetimePlays || 0,
      total_downloads: lifetimeDownloads || 0,
      total_likes: lifetimeLikes || 0,
      total_shares: lifetimeShares || 0,
      total_add_to_playlist: lifetimePlaylistAdds || 0,
      total_youtube_opens: lifetimeYoutubeOpens || 0,
