...
DO $$
BEGIN
  -- NOTE: some environments already have a table/view/materialized view named unified_analytics.
  -- Only create the view if no relation exists with that name.
  IF NOT EXISTS (
    SELECT 1
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'public'
      AND c.relname = 'unified_analytics'
  ) THEN
    EXECUTE '
