...
export async function uploadBanner(c: Context) {
  try {
    ...
    const insertData: any = {
      ...
      optimized_size_bytes: file.size, // Initial optimization is same as original
    };
    ...
    // Track for storage analytics
    try {
      const { track_file_upload } = await import("./storage-analytics-utils.tsx");
      ...
      console.warn("[Banner Upload] Storage tracking failed (non-fatal):", e);
    }
...
export async function uploadWallpaper(c: Context) {
  try {
    ...
    try {
      ...
      console.warn("[Wallpaper Upload] Storage tracking failed (non-fatal):", e);
    }
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        publish_status: publishStatus,
        published_at: publishStatus === "published" ? new Date().toISOString() : null,
        original_size_bytes: file ? file.size : 0,
        optimized_size_bytes: file ? file.size : 0,
      })
      .select()
      .single();
    ...
    if (file && storagePath) {
      try {
        ...
        await track_file_upload(
          ...
          title,
          "media",
          actualMediaType === "audio" ? "audio" : "video",
          file.size,
          file.size,
          { artist }
        );
      } catch (e) {
        console.warn("[Media Upload] Storage tracking failed (non-fatal):", e);
      }
    }
  ...
  try {
    ...
    if (file && storagePath) {
      try {
        ...
        console.warn("[Sparkle Upload] Storage tracking failed (non-fatal):", e);
      }
    }
